...
func AssertSuccessfulResponse() ResponseAssertion {
	return func(t *ctf.T, entities Entities, response *pb.Customer, err error) {
		require.NoError(t, err, "there should be no error while finding customer by PESEL")
		require.NotNil(t, response, "response should not be nil while finding customer by PESEL")
	}
}
